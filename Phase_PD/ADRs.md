...
---
## ADR-034 — Sequential Verification Chain — Three-Role Architecture
**Status:** Locked
**Decision:** Verification of build artifacts requires role separation. Three roles: Builder (produces artifact + manifest), Verifier (runs cis_verify.py), Auditor (Layer 3 external frontier model via CIS Live). The model that built the artifact cannot serve as its own Layer 3 auditor. A different frontier model must perform that role. Chain does not advance until each role completes with PASS.
**Rationale:** LLMs are unreliable self-reporters of task completion. A model cannot objectively audit its own output. Role separation ensures independent verification at each layer and prevents confirmation bias from propagating through the build.
---
## ADR-035 — Local Model Stack — Multi-Model Architecture for Intelligence and Verification
**Status:** Locked
**Decision:** The local intelligence layer is a multi-model stack selected by task type, not a single default model. Target slots: Slot 1 — primary reasoning and coding (Qwen3-32B or equivalent), Slot 2 — verification and second opinion (DeepSeek-R1-32B or equivalent), Slot 3 — fast routing and drafts (Qwen3-30B-A3B MoE or equivalent). All slots must fit 24GB VRAM at Q4 and run at comfortable interactive speed. ADR-001 extraction model slot is unchanged. Model selection mechanism is the Intel sidebar LOCAL tab. Agentic orchestration across local models is a future phase target.
**Rationale:** Mirrors the frontier model philosophy already proven in CIS Live — cross-checking across models overcomes blind spots. Different models have different capability profiles; routing by task type produces better results than a single default model for all tasks.
---
## ADR-036 — Sovereign Local Intelligence — System Maturity Model
**Status:** Locked
